...
type (
	// The arg represents a left-hand operand
	// ParseFn must start moving
	// only if the token (associated with its ParseFn)
	// is set to p.curToken
	prefixParseFn func() ast.Expression // ++1
...
func (p *Parser) parseLetStatement() ast.Statement {
	...
	p.nextToken() // on where '"=", "p.curToken", "...", ...'
...
func (p *Parser) parseReturnStatement() ast.Statement {
	...
	p.nextToken() // on '"return", "p.curtoken", ...'
...
func (p *Parser) peekPrecedence() int {
	if pd, ok := precedence[p.peekToken.Type]; ok {
		return pd
	}
...
func (p *Parser) curPrecedence() int {
	if pd, ok := precedence[p.curToken.Type]; ok {
		return pd
	}
...
func (p *Parser) parseFunctionParameters() []*ast.Identifier {
	...
	for p.peekTokenIs(token.COMMA) {
		p.nextToken() // after: on the ','
		p.nextToken() // after: om the next identifier
		ident := &ast.Identifier{Token: p.curToken, Value: p.curToken.Literal}
		identifiers = append(identifiers, ident)
	}
